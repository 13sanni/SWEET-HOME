...
const AddRoom = () => {
  ...
  return (
    <form onSubmit={onSubmitHandler}>
        <Title align='left' font='Outfit' title='Add Room' subTitle="Fill in the room details carefully to enhance the user booking experience." />
        ...
        <div className='grid grid-cols-2 sm:flex gap-4 my-2 flex-wrap'>
            {Object.keys(images).map((key)=>(
                <label htmlFor={`roomImage${key}`} key={key}>
                    ...
                    <input type="file" accept='image/*' id={`roomImage${key}`} onChange={e=>setImages({...images,[key]:e.target.files[0]})}/>
                </label>
            ))}
        </div>
