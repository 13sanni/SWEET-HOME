        ...
        <form onSubmit={onSubmitHandler} onClick={(e)=>e.stopPropagation()} className='flex bg-white rounded-xl max-w-4xl max-md:mx-2'>
            ...
            <div className='relative flex flex-col items-center md:w-1/2 p-8 md:p-10'>
                ...
                <div className='w-full mt-4'>
                  ...
                  <input type="text" id='name' value={name} placeholder='Type here' required className='border border-gray-200 rounded w-full px-3 py-2.5 mt-1 outline-indigo-500 font-light' onChange={(e)=>{setName(e.target.value)}}/>
                </div>

                {/* Hotel Contact */}
                <div className='w-full mt-4'>
                  <label htmlFor="contact" className='font-medium text-gray-500'>Phone</label>
                  <input type="text" id='contact' value={contact} onChange={(e)=>{setContact(e.target.value)}} placeholder='Type here' required className='border border-gray-200 rounded w-full px-3 py-2.5 mt-1 outline-indigo-500 font-light'/>
                </div> 

                {/* Hotel Address */}
                <div className='w-full mt-4'>
                  <label htmlFor="address" className='font-medium text-gray-500'>Address</label>
                  <input type="text" id='address' value={address} onChange={(e)=>{setAddress(e.target.value)}} placeholder='Type here' required className='border border-gray-200 rounded w-full px-3 py-2.5 mt-1 outline-indigo-500 font-light'/>
                </div>
